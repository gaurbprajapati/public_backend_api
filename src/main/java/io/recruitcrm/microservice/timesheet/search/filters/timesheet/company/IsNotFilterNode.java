...
/**
 * Filter node for company IS_NOT filter - matches timesheets assigned to jobs with
 * companies other than the specified company ID. Excludes unassigned timesheets (only
 * shows assigned jobs that are not the specified company).
 */
public class IsNotFilterNode extends CompanyFieldBaseFilterNode {
	...
	public List<Condition> getFilterConditions() {
		...
		if (companyId == null) {
			// If no company ID provided, return no condition (matches all)
			return List.of();
		}

		// Return timesheets assigned to jobs with companies other than the specified
		// company
		// Exclude unassigned timesheets (job_id IS NOT NULL) and exclude the specified
		// company
		return List.of(TimesheetGroupBaseFilterNode.TS_SETTING_ASSOC.JOB_ID.isNotNull()
			.and(this.companyIdField.ne(companyId)));
	}
