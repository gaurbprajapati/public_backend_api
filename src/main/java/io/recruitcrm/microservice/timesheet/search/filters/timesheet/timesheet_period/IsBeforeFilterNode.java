...
/**
 * Filter node for timesheet period IS_BEFORE filter - matches timesheets where the entire
 * period (period_end) is before the specified epoch timestamp (in seconds).
 */
public class IsBeforeFilterNode extends TimesheetPeriodFieldBaseFilterNode {
	...
	public List<Condition> getFilterConditions() {
		...
		// Return timesheets where period_end < filterValue OR period_start < filterValue
		Condition periodBeforeValue = this.periodEndField.lt(filterValue).or(this.periodStartField.lt(filterValue));

		return List.of(periodBeforeValue);
	}
