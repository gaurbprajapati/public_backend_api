...
/**
 * Filter node for exact match logic with NOT IN - matches timesheets for contractor-job
 * pairs that do NOT have exactly the specified deals. Uses contractor-job pair filtering
 * approach to ensure both contractor and job are checked.
 */
public class IsNotFilterNode extends DistinctDealFilterNode {
	...
	public List<Condition> getFilterConditions() {
		...
		Table<?> pairTable = contractorJobPairsWithDeals.asTable("pair");
		...
		// Filter timesheets where the contractor-job pair is NOT IN the matching pairs
		...
		return List.of(DSL
			...
			.notIn(DSL.select(pairCandidateIdField, pairJobIdField)
				.from(pairTable)
