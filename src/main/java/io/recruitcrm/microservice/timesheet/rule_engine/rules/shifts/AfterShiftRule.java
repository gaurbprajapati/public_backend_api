...
/**
 * Rule for calculating after shift time
 */
public class AfterShiftRule extends BaseRule {
	...
	public AfterShiftRule(Logger logger) {
		super(logger);
	}
	...
	public RuleEvaluationResult evaluate(RuleEvaluationContext ruleEvaluationContext) {
		...
		Duration afterShiftDuration = TimeHelper
			.convertRangeSetToDuration(ruleEvaluationContext.getTimeRangesToEvaluate());

		// Calculate amounts based on charge method type
		BigDecimal payableAmount = TimeHelper.calculatePayAmount(afterShiftDuration, this.ruleChargeMethod,
				this.basePayRatePerHour, // base pay rate from timesheet settings
				this.rulePayRateMultiplier, this.rulePayRatePerHour);

		BigDecimal billableAmount = TimeHelper.calculateBillAmount(afterShiftDuration, this.ruleChargeMethod,
	...
	public String getName() {
		return "Range-Based After Shift Rule";
	}

	@Override
	protected RuleType getDefaultRuleType() {
		return RuleType.RANGE_BASED_AFTER_SHIFT;
	}
