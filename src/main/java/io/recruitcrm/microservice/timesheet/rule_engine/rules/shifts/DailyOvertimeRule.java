...
/**
 * Rule for calculating daily overtime hours based on shifts
 */
public class DailyOvertimeRule extends BaseRule {
	...
	public DailyOvertimeRule(Logger logger) {
		super(logger);
	}
	...
	public RuleEvaluationResult evaluate(RuleEvaluationContext ruleEvaluationContext) {
		...
		Duration dailyOvertimeDuration = TimeHelper
			.convertRangeSetToDuration(ruleEvaluationContext.getTimeRangesToEvaluate());

		// Calculate amounts based on charge method type
		BigDecimal payableAmount = TimeHelper.calculatePayAmount(dailyOvertimeDuration, this.ruleChargeMethod,
				this.basePayRatePerHour, // base pay rate from timesheet settings
				this.rulePayRateMultiplier, this.rulePayRatePerHour);

		BigDecimal billableAmount = TimeHelper.calculateBillAmount(dailyOvertimeDuration, this.ruleChargeMethod,
	...
	public String getName() {
		return "Range-Based Daily Overtime Rule";
	}
