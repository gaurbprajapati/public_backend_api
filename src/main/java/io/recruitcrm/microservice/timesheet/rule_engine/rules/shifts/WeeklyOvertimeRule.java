...
/**
 * Rule for calculating weekly overtime hours based on shifts
 */
public class WeeklyOvertimeRule extends BaseRule {
	...
	public String getName() {
		return "Range-Based Weekly Overtime Rule";
	}

	@Override
	protected RuleType getDefaultRuleType() {
		return RuleType.RANGE_BASED_WEEKLY_OVERTIME;
	}
	...
	private Duration getTotalHoursWorkedFromTimeLogList(List<TimeLog> timeLogs, RuleEvaluationContext context) {
		...
		return timeLogs.stream().filter((timeLog) -> {
			// Use the new utility method to check if time log has valid time data
			Duration duration = TimeHelper.calculateTimeLogDuration(timeLog);
			if (duration.isZero()) {
				this.logger.logWarn(MessageFormat.format("Skipping TimeLog with invalid time data: {0}", timeLog));
				return false;
			}
			return true;
		})
	...
	private Duration getTotalHoursWorkedFromRangeSetList(List<RangeSet<LocalTime>> rangeSets) {
		...
		return rangeSets.stream()
			.filter((rangeSet) -> rangeSet != null && !rangeSet.isEmpty())
			.map(TimeHelper::convertRangeSetToDuration)
			.reduce(Duration.ZERO, Duration::plus);
	}
