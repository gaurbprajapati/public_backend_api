...
class GroupANDNodeTests {
	...
	@BeforeEach
	void setUp() {
		this.groupANDNode = new GroupANDNode();
	...
	void testToSQLWithSingleSubGroup() {
		...
		this.groupANDNode.addChild(subGroup);

		Select<?> result = this.groupANDNode.toSQL(this.table, this.searchTableIdField);

		assertThat(result).isNotNull();
	}

	@Test
	@DisplayName("toSQL should create query with multiple subGroups using AND logic")
	void testToSQLWithMultipleSubGroups() {
		...
		this.groupANDNode.addChild(subGroup1);
		this.groupANDNode.addChild(subGroup2);
		this.groupANDNode.addChild(subGroup3);
	...
	@Test
	@DisplayName("toSQL should use AND condition for multiple subGroups")
	void testToSQLUsesAndCondition() {
		SubGroupConjointNode subGroup1 = mock(SubGroupConjointNode.class);
		SubGroupConjointNode subGroup2 = mock(SubGroupConjointNode.class);
		...
		// Verify AND logic is used (check for "and" in SQL)
	...
	void testAddChild() {
		...
		assertThat(this.groupANDNode.getSubGroups()).hasSize(SINGLE_SUBGROUP_COUNT);
		assertThat(this.groupANDNode.getSubGroups().get(0)).isEqualTo(subGroup);
	}
	...
	void testAddChildMultiple() {
		...
		assertThat(this.groupANDNode.getSubGroups()).hasSize(MULTIPLE_SUBGROUPS_COUNT);
	}
