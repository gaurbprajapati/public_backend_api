...
@DisplayName("ContractorJob DoesNotContainFilterNode Tests")
class DoesNotContainFilterNodeTests {
	...
	@DisplayName("Get filter conditions should return not-exists condition for valid filter values")
	void testGetFilterConditionsValidFilterValuesReturnNotExistsCondition(String filterValue) {
	...
	void testIsSelectDistinctReturnsTrue() {
		DoesNotContainFilterNode doesNotContainFilterNode = this.createNode("1,2,3");
		assertThat(doesNotContainFilterNode.isSelectDistinct()).isTrue();
	}
	...
	private List<Condition> getFilterConditionsForValue(String filterValue) {
		...
		return doesNotContainFilterNode.getFilterConditions();
	}
