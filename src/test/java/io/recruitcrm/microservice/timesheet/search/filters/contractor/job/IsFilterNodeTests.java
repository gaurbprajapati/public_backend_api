...
@DisplayName("ContractorJob IsFilterNode Tests")
class IsFilterNodeTests {
	...
	@DisplayName("Get filter conditions should return exact match condition for valid filter values")
	void testGetFilterConditionsValidFilterValuesReturnExactMatchCondition(String filterValue) {
	...
	void testGetJoinTablesReturnsMinimalJoins() {
		IsFilterNode isFilterNode = this.createNode("1,2,3");
	...
	void testIsSelectDistinctReturnsTrue() {
		IsFilterNode isFilterNode = this.createNode("1,2,3");
		assertThat(isFilterNode.isSelectDistinct()).isTrue();
	}
	...
	private List<Condition> getFilterConditionsForValue(String filterValue) {
		...
		return isFilterNode.getFilterConditions();
	}
