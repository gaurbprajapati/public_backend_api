...
class SubGroupORNodeTests {
	...
	@BeforeEach
	void setUp() {
		this.subGroupORNode = new SubGroupORNode();
	...
	void testToSQLWithSingleFilterIN() {
		...
		this.subGroupORNode.addChild(filterNode);

		Select<?> result = this.subGroupORNode.toSQL(this.dslContext, this.table, this.searchTableIdField);
	...
	@Test
	@DisplayName("toSQL should create query with multiple filters using OR logic with IN")
	void testToSQLWithMultipleFiltersIN() {
		...
		this.subGroupORNode.addChild(filterNode1);
		this.subGroupORNode.addChild(filterNode2);
		this.subGroupORNode.addChild(filterNode3);
		...
		assertThat(result).isNotNull();
		assertThat(sql).contains("or").contains("a0").contains("a1").contains("a2");
	}

	@Test
	@DisplayName("toSQL should create query with multiple filters using OR logic with NOT_IN")
	void testToSQLWithMultipleFiltersNotIn() {
		...
		String sql = result.toString().toLowerCase();
		assertThat(result).isNotNull();
		assertThat(sql).contains("or").contains("not in");
	}
	...
	void testAddChild() {
		...
		assertThat(this.subGroupORNode.getFilters()).hasSize(SINGLE_FILTER_COUNT);
		assertThat(this.subGroupORNode.getFilters().get(0)).isEqualTo(filterNode);
	}
	...
	void testAddChildMultiple() {
		...
		assertThat(this.subGroupORNode.getFilters()).hasSize(MULTIPLE_FILTERS_COUNT);
	}
