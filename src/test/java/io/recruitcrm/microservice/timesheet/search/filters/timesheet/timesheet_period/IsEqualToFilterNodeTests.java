...
@DisplayName("TimesheetPeriod IsEqualToFilterNode Tests")
class IsEqualToFilterNodeTests {
	...
	void testGetFilterConditionsValidFilterValueReturnsCondition(String filterValue) {
		...
		filterDto.setFilterType(FilterTypes.IS_EQUAL_TO);
		filterDto.setFilterValue(filterValue);
		filterDto.setGroupType("AND");

		this.filterNodeContext.setFilterDto(filterDto);
		IsEqualToFilterNode isEqualToFilterNode = new IsEqualToFilterNode(this.filterNodeContext);

		List<Condition> conditions = isEqualToFilterNode.getFilterConditions();
	...
	static Stream<String> provideValidFilterValues() {
		// epoch int (JSON number), JSON textual, padded, and plus-prefixed (invalid JSON,
		// parsed as long string) - all within int range so downstream toIntExact succeeds
		return Stream.of("1764527400", "\"1764527400\"", "  1764527400  ", "+1764527400");
	}
	...
	void testParseFilterValueJsonLongBeyondIntegerRangeReturnsLong() throws Exception {
		...
		Method parseFilterValue = IsEqualToFilterNode.class.getDeclaredMethod("parseFilterValue");
		parseFilterValue.setAccessible(true);
		Object result = parseFilterValue.invoke(isEqualToFilterNode);
	...
	void testIsSelectDistinctReturnsTrue() {
		...
		Boolean isDistinct = isEqualToFilterNode.isSelectDistinct();
